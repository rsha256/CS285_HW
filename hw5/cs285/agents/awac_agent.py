...
class AWACAgent(DQNAgent):
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        ob_no, ac_na, re_n, next_ob_no, terminal_n = ptu.from_numpy(ob_no), ptu.from_numpy(ac_na), ptu.from_numpy(re_n), ptu.from_numpy(next_ob_no), ptu.from_numpy(terminal_n)
        ...
        if self.agent_params['discrete']:
            p = dist.probs
            q = self.exploitation_critic.q_net(ob_no)
            # Vectorized sum
            v_pi = torch.sum(p * q, dim=1)
        else:
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            # 1): Update the exploration model (based of    f s')
