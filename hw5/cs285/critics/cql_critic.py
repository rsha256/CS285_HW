...
class CQLCritic(BaseCritic):
    ...
    def dqn_loss(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        if self.double_q:
            next_ac = self.q_net(next_ob_no).argmax(1)
            q_tp1_values = qa_tp1_values.gather(1, next_ac.unsqueeze(1)).squeeze(1)
        else:
            q_tp1_values = qa_tp1_values.max(1)[0]
        target = reward_n + self.gamma * (1 - terminal_n) * q_tp1_values
