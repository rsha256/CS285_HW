            ...
            if isinstance(self.agent, MBPOAgent):
                for _ in range(self.sac_params['n_iter']):
                    if self.params['mbpo_rollout_length'] > 0:
                        ...
                        self.agent.add_to_replay_buffer(self.agent.collect_model_trajectory(self.params['mbpo_rollout_length']), from_model=True)
                    # train the SAC agent
    ...
    def collect_training_trajectories(self, itr, initial_expertdata, collect_policy, num_transitions_to_sample, save_expert_data_to_disk=False):
        ...
        if itr == 0:
            ...
            if isinstance(self.agent, SACAgent):
                ...
                paths, envsteps_this_batch = utils.sample_random_trajectories(self.env, num_transitions_to_sample, self.params['ep_len'])
