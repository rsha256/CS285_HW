...
class MPCPolicy(BasePolicy):
    ...
    def sample_action_sequences(self, num_sequences, horizon, obs=None):
        ...
        if self.sample_strategy == 'random' \
            ...
            mu = candidate_action_sequences.mean(axis=0)
            var = candidate_action_sequences.var(axis=0)
            for i in range(self.cem_iterations):
                ...
                var = self.cem_alpha * np.var(elite_action_sequences, axis=0) + (1 - self.cem_alpha) * var
                candidate_action_sequences = np.random.normal(loc=mu, scale=np.sqrt(var), size=(num_sequences, horizon, self.ac_dim))
                candidate_action_sequences = candidate_action_sequences.clip(self.low, self.high)
